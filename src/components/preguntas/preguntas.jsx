...
export const Preguntas = () => {
  ...
  return (
    <div className="p-16 mq980:p-8">
      <h1 className="text-4xl text-gray-700 font-bold text-center mb-16 mq980:mb-8">
        PREGUNTAS FRECUENTES
      </h1>
      <div className="grid grid-cols-3 gap-6 mq980:grid-cols-1">
        {preguntas.map((item, index) => (
          <div key={index} className={`${item.color} p-4 rounded-xl shadow-lg`}>
            <button
              onClick={() => toggleAnswer(index)}
              className="text-left w-full text-xl font-semibold text-gray-700 py-3 px-4 rounded-xl flex justify-between items-center"
            >
              {item.pregunta}
              <IoIosArrowDown
                className={`${
